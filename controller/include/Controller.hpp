...
//    grabs frames from Avedio USB capture → displays on 5" HDMI screen
//
// Shared state between gamepad and radio loops: three atomic<float> duty values.
// No mutex needed — a single stale value on one 40 Hz send is imperceptible.
class Controller {
public:
    Controller();
    bool init();
...
private:
    ...
    static constexpr float THROTTLE_CAP = 0.65f;   // fraction of 2.5% range
};
