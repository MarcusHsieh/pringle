...
void Controller::run() {
    // gamepad and radio on background threads, video on main thread
...
// mixInputs
// Differential drive
// THROTTLE_CAP limits range to 65% of [5.0, 10.0]
Controller::Duties Controller::mixInputs(const GamepadState& g) const {
    ...
    return Duties{ toDuty(left), toDuty(right), toDuty(g.servoPos) };
}
